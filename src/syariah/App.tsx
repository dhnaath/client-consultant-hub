  ...
  useEffect(() => {
    ...
    const fetchLiveIndices = async () => {
      try {
        ...
        setCommodities((prev) =>
          prev.map((c) => {
            if (c.id === "ihsg" && data.ihsg) {
              return {
                ...c,
                currentPrice: data.ihsg.price,
                change24h: data.ihsg.change,
                changePercent24h: data.ihsg.changePercent,
              };
            }
            if (c.id === "jii" && data.jii) {
              return {
                ...c,
                currentPrice: data.jii.price,
                change24h: data.jii.change,
                changePercent24h: data.jii.changePercent,
              };
            }
            if (c.id === "isi" && data.isi) {
              return {
                ...c,
                currentPrice: data.isi.price,
                change24h: data.isi.change,
                changePercent24h: data.isi.changePercent,
              };
            }
            if (c.id === "sp500sh" && data.sp500sh) {
              return {
                ...c,
                currentPrice: data.sp500sh.price,
                change24h: data.sp500sh.change,
                changePercent24h: data.sp500sh.changePercent,
              };
            }
        ...
        // Silently ignore fetch errors in production to prevent console noise,
        // as we already have fallback MOCK_COMMODITIES data loaded.
        console.warn("Live indices currently unavailable, using fallback data.");
      }
    };
